...
class Migration4To5Test {
    ...
    @Test fun `explicit chain through V5 preserves V5 contract in current schema`() {
        ...
        try { db.execSQL("INSERT INTO career_player_club_season_stats (careerId,playerId,legacySeasonId,legacyClubId,legacyC,legacyD,legacyE,legacyF,legacyG,legacyH) VALUES ('career-v5','missing',1,101,0,0,0,0,0,0)"); fail("Expected player runtime foreign key rejection") } catch (_: SQLiteConstraintException) {}
        ...
        db.query("SELECT legacyLeagueSubtype FROM career_competitions").use { c -> assertTrue(!c.moveToFirst()) }
        db.close()
    }
}

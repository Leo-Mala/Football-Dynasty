...
class Migration7To8Test {
    ...
    @Test fun `V7 to V8 stadium step remains fail closed through current schema`() {
        val name="marco-b-migration-3-current-v8-contract"
        var db=helper.createDatabase(name,3)
        db.execSQL("INSERT INTO career_metadata (id,dataVersion,displayName,legacyMetadataFingerprint,legacyCareerFingerprint,createdAtEpochMillis,updatedAtEpochMillis) VALUES ('career-v8',1,'Migration V8 probe',NULL,NULL,10,10)")
        db.close()
        ...
        db.query("SELECT displayName FROM career_metadata WHERE id='career-v8'").use { c -> assertTrue(c.moveToFirst()); assertEquals("Migration V8 probe",c.getString(0)) }

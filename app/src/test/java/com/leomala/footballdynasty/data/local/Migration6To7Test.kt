...
class Migration6To7Test {
    ...
    @Test fun `explicit migration chain preserves V7 manager state contract through current schema`() {
        ...
        expectedTables.forEach { table -> db.query("SELECT COUNT(*) FROM `$table`").use { c -> assertTrue(c.moveToFirst()); assertEquals("Migration must not synthesize $table rows",0,c.getInt(0)) } }
        ...
        for (table in listOf("career_club_ticket_runtime","career_manager_ticket_runtime","career_match_construction_source")) db.query("SELECT COUNT(*) FROM `$table`").use { c -> assertTrue(c.moveToFirst()); assertEquals("V9 must not synthesize $table rows",0,c.getInt(0)) }
        for (table in listOf("career_coach_runtime","career_coach_season_club_records")) db.query("SELECT COUNT(*) FROM `$table`").use { c -> assertTrue(c.moveToFirst()); assertEquals("V11 must not synthesize $table rows",0,c.getInt(0)) }

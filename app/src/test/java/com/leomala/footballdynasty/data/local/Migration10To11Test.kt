...
class Migration10To11Test {
    @Test
    fun `migration to 11 preserves V10 H identity and creates no invented coach state`() = runBlocking {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val name = "phase14-migration-10-11"
        ...
        current.careerMetadataDao().upsert(
            CareerMetadataEntity(
                id = CAREER,
                dataVersion = 1,
                displayName = "Migration V11 probe",
        ...
        CareerTicketRuntimeStore(current).materializeManagers(
            CAREER,
            listOf(CareerManagerTicketRuntimeState(0, 7, 61)),
        )
        current.close()
        ...
        assertNull(coach.find(CAREER, 0))
        assertNull(migrated.careerCoachRuntimeDao().findCoachRuntime(CAREER, 0))
        assertTrue(migrated.careerCoachRuntimeDao().seasonClubRecords(CAREER, 0).isEmpty())
        assertEquals("Migration V11 probe", migrated.careerMetadataDao().findById(CAREER)?.displayName)
    ...
    private companion object {
        const val CAREER = "career-v11"
        const val V10_IDENTITY = "086ae14fb0b6a625e6f22446d47df0b7"
    }
}

...
class Migration5To6Test {
    ...
    @Test fun `explicit chain through V6 preserves competition runtime in current schema`() {
        ...
        db.query("PRAGMA table_info('career_competition_standings')").use { c -> val names=mutableSetOf<String>(); val i=c.getColumnIndex("name"); while(c.moveToNext()) names+=c.getString(i); assertEquals(setOf("careerId","competitionId","clubId","stableOrdinal","points","played","wins","losses","goalsFor","goalsAgainst"),names) }
        db.query("PRAGMA table_info('career_competition_matches')").use { c -> val names=mutableSetOf<String>(); val i=c.getColumnIndex("name"); while(c.moveToNext()) names+=c.getString(i); assertEquals(setOf("careerId","competitionId","matchId","roundNumber","fixtureOrdinal"),names) }

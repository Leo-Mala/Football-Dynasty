...
interface CareerManagerRuntimeDao {
    @Upsert suspend fun upsertPlayerCommercial(entity: CareerPlayerCommercialEntity)
    @Upsert suspend fun upsertPlayerTransferState(entity: CareerPlayerTransferStateEntity)
    @Upsert suspend fun upsertClubRuntime(entity: CareerClubManagerRuntimeEntity)
    @Upsert suspend fun upsertActiveLoan(entity: CareerActiveLoanEntity)
    @Upsert suspend fun upsertStadiumConstruction(entity: CareerStadiumConstructionEntity)
    @Upsert suspend fun upsertStadiumConstructions(entities: List<CareerStadiumConstructionEntity>)
    @Upsert suspend fun upsertStadiumRuntime(entity: CareerStadiumRuntimeEntity)

    @Query("SELECT * FROM career_player_commercial WHERE careerId = :careerId AND playerId = :playerId LIMIT 1")
    suspend fun findPlayerCommercial(careerId: String, playerId: String): CareerPlayerCommercialEntity?
    @Query("SELECT * FROM career_player_transfer_state WHERE careerId = :careerId AND playerId = :playerId LIMIT 1")
    ...
    @Query("SELECT * FROM career_club_manager_runtime WHERE careerId = :careerId AND clubId = :clubId LIMIT 1")
    ...
    @Query("SELECT * FROM career_active_loans WHERE careerId = :careerId AND playerId = :playerId LIMIT 1")
    ...
    @Query("SELECT * FROM career_stadium_constructions WHERE careerId = :careerId ORDER BY sourceOrdinal")
    ...
    @Query("SELECT * FROM career_stadium_runtime WHERE careerId = :careerId AND clubId = :clubId LIMIT 1")
    suspend fun findStadiumRuntime(careerId: String, clubId: String): CareerStadiumRuntimeEntity?
}
